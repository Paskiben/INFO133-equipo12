...
for u in URL_SEED:
	...
	all_urls = response.html.xpath("//div//h3/a/@href")[:3]

	# NOTICIAS
	for c, url in enumerate(all_urls):
              ...
              # fecha
              date = response.html.xpath("//article/div[3]/p[2]")[0].text
              print(date)
            
              # contenido
              list_p = response.html.xpath("//div[@class='entry-content entry clearfix']//p")
              text=""
              for p in list_p:
                content = p.text
